	...
	try {
		...
		for (; $tmp3;) {
			...
			if ($tmp4) {
				...
				$Λ[$Λ.length - 1] = { 'l': $lub($old_pc.l, sec_lvl('j', null, true, $Γ['global']['chkpassword'])) };
				throw j;
			} else {
			}
			$comp({ 'lbl': 'TRY' }, $Λ[$Λ.length - 1].l);
			$Λ.pop();
			$tmp2 = j++;
		...
		$Γ['global']['chkpassword']['len'] = len instanceof Object ? { '\u03A3': $Λ[$Λ.length - 1].l } : $Λ[$Λ.length - 1].l;
		var $tmp1;
		$tmp1 = $output('len',len,$Γ['global']['chkpassword'],0,'suppress',$Λ[$Λ.length - 1].l);
		$Λ.pop();
	}
